...
use zcash_vote::{
    as_byte256, ballot::Ballot, db::store_dnf, election::{Frontier, OrchardHash}, Election
};
...
pub fn get_ballot_height(id: String, height: u32, state: &State<Context>) -> Result<Json<Value>, Custom<String>> {
    (|| {
        let connection = state.pool.get()?;
...
pub fn post_ballot(id: String, ballot: Json<Ballot>, state: &State<Context>) -> Result<String, Custom<String>> {
    let res = || {
        ...
        let data = zcash_vote::validate::validate_ballot(ballot.clone().into_inner(), election.signature_required)?;
        println!("Validated");
